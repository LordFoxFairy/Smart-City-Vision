...
def perform_image_inpainting_deep_learning(image_path, mask_coords, prompt_text="a clean image without any defects"):
    ...
    mask_coords (tuple): 掩码区域的坐标 (x1, y1, x2, y2)。
    prompt_text (str): 描述修复区域内容的文本提示。

    返回:
    tuple: (原始图像 numpy 数组, 掩码图像 numpy 数组, 修复后的图像 numpy 数组)。
    """
    try:
        ...
        # 创建掩码
        # 掩码是一个单通道图像，白色 (255) 表示需要修复的区域，黑色 (0) 表示保留的区域
        mask_pil = Image.new('L', image_pil.size, 0) # 创建全黑掩码
        mask_draw = ImageDraw.Draw(mask_pil)
        mask_draw.rectangle(mask_coords, fill=255) # 在指定坐标绘制白色矩形区域

        # 运行图像修复管道
        # prompt_text 引导模型生成什么内容
        # image 是原始图像
        # mask_image 是掩码
        # guidance_scale 控制模型对 prompt 的遵循程度
        # num_inference_steps 控制生成质量和速度
...
def find_semantic_outliers_kmeans(image_dir, num_clusters=3, outlier_cluster_size_threshold=2):
    ...
    参数:
    image_dir (str): 图片所在的目录。
    num_clusters (int): K-Means聚类的簇数量。
    outlier_cluster_size_threshold (int): 判定为离群簇的最大图片数量。
    ...
    # 确保 n_init 是 'auto' 或一个整数，以避免未来版本警告
...
if not semantic_outliers_kmeans:
    ...
    if semantic_outliers_kmeans:
        print("\n可视化检测到的语义离群点：")
        fig, axes = plt.subplots(1, len(semantic_outliers_kmeans), figsize=(5 * len(semantic_outliers_kmeans), 5))
        if len(semantic_outliers_kmeans) == 1: # 如果只有一个，axes不是数组
            axes = [axes]
...
def evaluate_pre_annotation_risk(image_dir, target_labels, irrelevant_threshold=0.6, ambiguity_threshold=0.05):
    ...
    参数:
    image_dir (str): 图片所在的目录。
    target_labels (list): 任务的目标文本标签列表 (e.g., ["一只猫", "一只狗"])。
    ...
    for i, img_path in enumerate(image_paths):
        ...
        if top1_score < irrelevant_threshold:
            ...
            print(f"- '{os.path.basename(img_path)}': 潜在异常 (最高相似度 {top1_score:.2f} 对 '{top1_label}')")
        else:
            # 模糊样本判断 (如果不是潜在异常)
            if len(target_labels) > 1: # 只有当有多个标签时才判断模糊性
                ...
                if (top1_score - top2_score) < ambiguity_threshold:
                    ...
                    print(f"- '{os.path.basename(img_path)}': 模糊样本 (对 '{top1_label}' 相似度 {top1_score:.2f}, 对 '{top2_label}' 相似度 {top2_score:.2f})")
            # else:
            #     print(f"- '{os.path.basename(img_path)}': 正常 (最高相似度 {top1_score:.2f} 对 '{top1_label}')")
    
    return potential_irrelevant_samples, ambiguous_samples
...
# 定义任务目标标签
# 假设我们的任务是识别猫和狗
target_labels_for_task = ["一只猫", "一只狗"] # 增加一个潜在的离群标签
# target_labels_for_task = ["一只猫", "一只狗", "一辆汽车", "一片风景"] # 更多标签

# 调整阈值以适应示例数据
# '08_semantic_outlier.jpg' (风景) 应该被识别为潜在异常
# '03_semantic_duplicate.jpg' (另一只猫) 应该被识别为正常或略模糊
irrelevant_thresh = 0.25 # 如果最高相似度低于0.25，认为是无关的
ambiguity_thresh = 0.05 # 如果最高和次高相似度差值小于0.05，认为是模糊的

irrelevant_samples, ambiguous_samples = evaluate_pre_annotation_risk(
    DEMO_IMAGES_DIR, 
...
if irrelevant_samples:
    ...
    fig, axes = plt.subplots(1, len(irrelevant_samples), figsize=(5 * len(irrelevant_samples), 5))
    if len(irrelevant_samples) == 1: axes = [axes]
...
if ambiguous_samples:
    ...
    fig, axes = plt.subplots(1, len(ambiguous_samples), figsize=(5 * len(ambiguous_samples), 5))
    if len(ambiguous_samples) == 1: axes = [axes]
    for i, (path, s1, l1, s2, l2) in enumerate(ambiguous_samples):
        axes[i].imshow(Image.open(path))
        axes[i].set_title(f"Ambiguous:\n{os.path.basename(path)}\n(S1:{s1:.2f} to '{l1}', S2:{s2:.2f} to '{l2}')")
        axes[i].axis('off')
...
transform = A.Compose([
    ...
    A.RandomResizedCrop(height=image_rgb.shape[0], width=image_rgb.shape[1], scale=(0.8, 1.0), p=0.5),
])
